...
var D = (m, e, t) => e in m ? z(m, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : m[e] = t;
var s = (m, e, t) => D(m, typeof e != "symbol" ? e + "" : e, t);
function G(m) {
  ...
  calculateOverlayDimensions(e, t, i, r, o) {
    ...
    if (e.position.x !== void 0)
      if (typeof e.position.x == "string" && e.position.x.startsWith("right-")) {
        const c = parseInt(e.position.x.substring(6), 10), d = e.size.width !== void 0 && e.size.width !== "auto" ? P(e.size.width, this.canvasWidth) : r;
        h = this.canvasWidth - d - c;
      } else
        h = P(e.position.x, this.canvasWidth);
    return e.position.y !== void 0 && (a = P(e.position.y, this.canvasHeight)), e.size.width !== void 0 && e.size.width !== "auto" && (n = P(e.size.width, this.canvasWidth)), e.size.height !== void 0 && e.size.height !== "auto" && (f = P(e.size.height, this.canvasHeight)), { x: h, y: a, width: n, height: f };
  }
}
class X {
  constructor(e, t, i) {
  ...
  drawGridLabels(e, t, i) {
    ...
    for (let c = 0; c <= a; c++) {
      ...
      if (l === 0)
        ...
        const w = N(Math.abs(l)), y = l > 0 ? "+" : "-", v = Math.abs(w);
        v >= 100 ? u = `${y}${v.toFixed(0)}dB` : u = `${y}${v.toFixed(1)}dB`;
      }
...
class Q {
  ...
  drawWaveform(e, t, i, r) {
    ...
    for (let c = 0; c < o; c++) {
      const d = t + c, g = e[d], l = a - g * f, u = Math.min(this.canvasHeight, Math.max(0, l)), w = c * h;
...
class V extends O {
  ...
  drawFFTOverlay(t, i, r, o, h, a) {
    const n = r / o, f = Math.floor(this.canvasWidth * 0.35), c = Math.floor(this.canvasHeight * 0.35), d = 10, g = this.canvasHeight - c - 10, { x: l, y: u, width: w, height: y } = this.calculateOverlayDimensions(
      a,
      d,
      g,
      f,
    ...
    this.ctx.save(), this.ctx.fillStyle = "rgba(0, 0, 0, 0.7)", this.ctx.fillRect(l, u, w, y), this.ctx.strokeStyle = "#00aaff", this.ctx.lineWidth = 2, this.ctx.strokeRect(l, u, w, y);
    const v = Math.min(
      t.length,
      Math.ceil(h / n)
    ), p = w / v;
    ...
    for (let T = 0; T < v; T++) {
      const b = t[T] / 255 * y * this.FFT_OVERLAY_HEIGHT_RATIO, R = l + T * p, C = u + y - b;
      this.ctx.fillRect(R, C, Math.max(p - 1, this.FFT_MIN_BAR_WIDTH), b);
    }
    if (i > 0 && i <= h) {
      const T = i / n, S = l + T * p;
      this.ctx.strokeStyle = "#ff00ff", this.ctx.lineWidth = 2, this.ctx.beginPath(), this.ctx.moveTo(S, u), this.ctx.lineTo(S, u + y), this.ctx.stroke(), this.ctx.fillStyle = "#ff00ff", this.ctx.font = "bold 12px Arial";
      const b = `${i.toFixed(1)} Hz`, R = this.ctx.measureText(b).width;
      let C = S + 3;
      C + R > l + w - 5 && (C = S - R - 3), this.ctx.fillText(b, C, u + 15);
    }
    this.ctx.restore();
  }
}
class j extends O {
  ...
  drawHarmonicAnalysis(e, t, i, r, o, h, a, n) {
    ...
    (e !== void 0 ? 1 : 0) + (t ? 1 : 0) + (i ? 1 : 0) + (h ? 2 : 0)) * f + 10, { x: g, y: l, width: u, height: w } = this.calculateOverlayDimensions(
      n,
      10,
      10,
      500,
    ...
    let y = l;
    if (this.ctx.save(), this.ctx.fillStyle = "rgba(0, 0, 0, 0.8)", this.ctx.fillRect(g, l, u, w), this.ctx.strokeStyle = "#ffaa00", this.ctx.lineWidth = 2, this.ctx.strokeRect(g, l, u, w), this.ctx.fillStyle = "#ffaa00", this.ctx.font = "bold 12px monospace", y += 15, this.ctx.fillText("倍音分析 (Harmonic Analysis)", g + 5, y), e !== void 0 && a) {
      y += f, this.ctx.fillStyle = "#00ff00", this.ctx.font = "11px monospace";
      const v = a / 2;
      this.ctx.fillText(
    ...
    if (t && a) {
      y += f, this.ctx.fillStyle = "#ff00ff", this.ctx.font = "11px monospace";
      const v = t.map((T, S) => `${S + 1}x:${T.toFixed(2)}`).join(" "), p = r !== void 0 ? ` (重み付け: ${r.toFixed(1)})` : "";
      this.ctx.fillText(
    ...
    if (i && a) {
      y += f, this.ctx.fillStyle = "#00aaff", this.ctx.font = "11px monospace";
      const v = a / 2, p = i.map((S, b) => `${b + 1}x:${S.toFixed(2)}`).join(" "), T = o !== void 0 ? ` (重み付け: ${o.toFixed(1)})` : "";
      this.ctx.fillText(
        `候補2 (${v.toFixed(1)}Hz) 倍音: ${p}${T}`,
        g + 5,
        y
      );
    }
    if (h) {
      y += f, this.ctx.fillStyle = "#aaaaaa", this.ctx.font = "10px monospace";
      const v = u - 10, p = h.split(" ");
      let T = "";
      for (const S of p) {
        const b = T + (T ? " " : "") + S;
        this.ctx.measureText(b).width > v && T ? (this.ctx.fillText(T, g + 5, y), y += f, T = S) : T = b;
      }
      T && this.ctx.fillText(T, g + 5, y);
    }
    this.ctx.restore();
  }
}
class J extends O {
  ...
  drawFrequencyPlot(t, i, r, o) {
    ...
    const c = h + 35, d = a + 25, g = n - 45, l = f - 45, u = t.filter((x) => x > 0);
    if (u.length === 0) {
      this.ctx.restore();
      return;
    }
    const w = Math.min(...u), y = Math.max(...u), v = (y - w) * this.FREQ_PLOT_RANGE_PADDING_RATIO || this.FREQ_PLOT_MIN_RANGE_PADDING_HZ, p = Math.max(i, w - v), T = Math.min(r, y + v);
    ...
    for (let x = 0; x <= 4; x++) {
      const E = T - (T - p) * (x / 4), A = d + l / 4 * x, M = E >= 1e3 ? `${(E / 1e3).toFixed(1)}k` : `${E.toFixed(0)}`;
      this.ctx.fillText(M, c - 5, A);
    }
    ...
    this.ctx.strokeStyle = "#00ff00", this.ctx.lineWidth = 2, this.ctx.beginPath();
    const S = g / Math.max(t.length - 1, 1), b = Math.max(1, Math.floor(t.length / 4)), R = (x) => {
      const A = (Math.max(p, Math.min(T, x)) - p) / (T - p);
      return d + l - A * l;
    };
    let C = !1;
    for (let x = 0; x < t.length; x++) {
      ...
      const M = R(E);
      C ? this.ctx.lineTo(A, M) : (this.ctx.moveTo(A, M), C = !0);
    }
...
class ee {
  ...
  drawPhaseMarkers(e, t, i, r, o, h, a) {
    ...
    if (i !== void 0 && f(i, "#ff8800", 2), r !== void 0 && f(r, "#ff8800", 2), e !== void 0 && (f(e, "#ff0000", 2), this.debugOverlaysEnabled && (a == null ? void 0 : a.phaseZeroSegmentRelative) !== void 0)) {
      const d = (e - o) / n * this.canvasWidth, g = 20;
...
class te {
  constructor(e, t) {
    ...
    this.ctx = i, this.overlaysLayout = t || U, this.gridRenderer = new X(this.ctx, e.width, e.height), this.waveformLineRenderer = new Q(this.ctx, e.width, e.height), this.fftOverlayRenderer = new V(this.ctx, e.width, e.height), this.harmonicAnalysisRenderer = new j(this.ctx, e.width, e.height), this.frequencyPlotRenderer = new J(this.ctx, e.width, e.height), this.phaseMarkerRenderer = new ee(this.ctx, e.width, e.height, this.debugOverlaysEnabled), e.width === 300 && e.height === 150 && console.warn(
...
class re {
  ...
  drawWaveform(e, t, i, r, o, h, a) {
    ...
    for (let l = 0; l < n; l++) {
      const u = o + l;
      if (u >= r.length) break;
      const w = r[u], y = d - w * g, v = Math.min(i, Math.max(0, y)), p = l * c;
      l === 0 ? e.moveTo(p, v) : e.lineTo(p, v);
    }
    e.stroke();
  }
  /**
...
class ne {
  ...
  drawSimilarityPlot(e, t, i, r) {
    ...
    for (let l = 0; l <= 4; l++) {
      const u = c - (c - f) * (l / 4), w = h + n / 4 * l, y = u.toFixed(2);
      e.fillText(y, o - 5, w);
    }
    e.strokeStyle = "#00aaff", e.lineWidth = 2, e.beginPath();
    const d = a / Math.max(r.length - 1, 1);
    for (let l = 0; l < r.length; l++) {
      const u = r[l], w = o + l * d, v = (Math.max(f, Math.min(c, u)) - f) / (c - f), p = h + n - v * n;
...
class oe {
  ...
  drawOffsetOverlayGraphs(e, t, i, r = [], o = []) {
    ...
    const c = 2, d = n + c, g = f + 12, l = h - c * 2, u = a - 12 - c, w = 0, y = 100, v = (p, T) => {
      if (p.length < 2) return;
      e.strokeStyle = T, e.lineWidth = 1.5, e.beginPath();
      const S = l / Math.max(p.length - 1, 1);
      for (let b = 0; b < p.length; b++) {
        const R = p[b], C = d + b * S, x = (Math.max(w, Math.min(y, R)) - w) / (y - w), E = g + u - x * u;
        b === 0 ? e.moveTo(C, E) : e.lineTo(C, E);
      }
      e.stroke();
    };
    if (v(r, "#ff0000"), v(o, "#ff8800"), e.font = "8px monospace", e.textAlign = "left", r.length > 0) {
...
class he {
  constructor(e, t, i, r) {
    ...
    this.previousCtx = o, this.currentCtx = h, this.similarityCtx = a, this.bufferCtx = n, this.waveformRenderer = new re(), this.similarityPlotRenderer = new ne(), this.positionMarkerRenderer = new ae(), this.offsetOverlayRenderer = new oe(), this.clearAllCanvases();
  }
  /**
...
class le {
  ...
  drawSimilarityGraph(e, t, i, r, o, h) {
    if (e.save(), e.fillStyle = "#000000", e.fillRect(0, 0, t, i), e.strokeStyle = "#ff8800", e.lineWidth = 2, e.strokeRect(0, 0, t, i), e.fillStyle = "#ff8800", e.font = "bold 12px Arial", e.fillText(o, 5, 15), !r || r.length === 0) {
      e.fillStyle = "#666666", e.font = "11px Arial", e.fillText("データなし (No data)", t / 2 - 50, i / 2), e.restore();
      return;
    }
    const a = 35, n = 25, f = t - 45, c = i - 35, d = -1, g = 1;
    e.strokeStyle = "#333333", e.lineWidth = 1, e.beginPath();
    for (let y = 0; y <= 4; y++) {
      const v = n + c / 4 * y;
      e.moveTo(a, v), e.lineTo(a + f, v);
    }
    for (let y = 0; y <= r.length; y++) {
      const v = a + f / r.length * y;
      e.moveTo(v, n), e.lineTo(v, n + c);
    }
    e.stroke(), e.fillStyle = "#aaaaaa", e.font = "10px monospace", e.textAlign = "right", e.textBaseline = "middle";
    for (let y = 0; y <= 4; y++) {
      const v = g - (g - d) * (y / 4), p = n + c / 4 * y, T = v.toFixed(1);
      e.fillText(T, a - 5, p);
    }
    const l = f / r.length, u = l * 0.15;
    for (let y = 0; y < r.length; y++) {
      const v = r[y], p = Math.max(d, Math.min(g, v)), T = (p - d) / (g - d), S = n + c - T * c, b = n + c - (0 - d) / (g - d) * c, R = a + y * l + u, C = l - u * 2;
      v >= 0.9 ? e.fillStyle = "#00ff00" : v >= 0.7 ? e.fillStyle = "#88ff00" : v >= 0.5 ? e.fillStyle = "#ffaa00" : v >= 0 ? e.fillStyle = "#ff6600" : e.fillStyle = "#ff0000", S < b ? e.fillRect(R, S, C, b - S) : e.fillRect(R, b, C, S - b), e.fillStyle = "#ffffff", e.font = "9px monospace", e.textAlign = "center", p >= 0 ? (e.textBaseline = "bottom", e.fillText(v.toFixed(2), R + C / 2, S - 2)) : (e.textBaseline = "top", e.fillText(v.toFixed(2), R + C / 2, S + 2));
    }
    const w = n + c - (0 - d) / (g - d) * c;
    e.strokeStyle = "#666666", e.lineWidth = 1, e.beginPath(), e.moveTo(a, w), e.lineTo(a + f, w), e.stroke(), e.fillStyle = "#aaaaaa", e.font = "9px Arial", e.textAlign = "center", e.textBaseline = "top";
    for (let y = 0; y < r.length; y++) {
      const v = a + (y + 0.5) * l;
      e.fillText(`${y + 1}-${y + 2}`, v, n + c + 2);
    }
...
class fe {
  ...
  constructor(e, t, i, r, o) {
    ...
    this.audioManager = e, this.gainController = t, this.frequencyEstimator = i, this.waveformSearcher = r, this.zeroCrossDetector = o, this.basePathResolver = new W(), this.wasmLoader = new ce();
  }
  /**
  ...
  updatePhaseOffsetHistory(e) {
    ...
    if (!(t <= 0)) {
      if (e.phaseZeroIndex !== void 0) {
        const r = (e.phaseZeroIndex - e.displayStartIndex) / t * 100;
      ...
      if (e.phaseTwoPiIndex !== void 0) {
        const r = (e.phaseTwoPiIndex - e.displayStartIndex) / t * 100;
  ...
  reset() {
    const e = this.wasmLoader.getProcessor();
    e && e.reset(), this.phaseZeroOffsetHistory = [], this.phaseTwoPiOffsetHistory = [];
  }
}
class ue {
  ...
  constructor(e, t, i, r, o, h, a, n, f) {
    ...
    this.audioManager = new $(), this.gainController = new K(), this.frequencyEstimator = new Z(), this.renderer = new te(e, f), this.zeroCrossDetector = new ie(), this.waveformSearcher = new se(), this.comparisonRenderer = new he(
      t,
      i,
      r,
      o
    ), h && a && n && (this.cycleSimilarityRenderer = new le(
      h,
      a,
      n
...
export {
  $ as AudioManager,
  L as BufferSource,
  he as ComparisonPanelRenderer,
  le as CycleSimilarityRenderer,
  U as DEFAULT_OVERLAYS_LAYOUT,
  Y as FrameBufferHistory,
  Z as FrequencyEstimator,
  K as GainController,
  oe as OffsetOverlayRenderer,
  ue as Oscilloscope,
  me as PianoKeyboardRenderer,
  ae as PositionMarkerRenderer,
  ne as SimilarityPlotRenderer,
  fe as WaveformDataProcessor,
  re as WaveformPanelRenderer,
  te as WaveformRenderer,
  se as WaveformSearcher,
  ie as ZeroCrossDetector,
  N as amplitudeToDb,
  G as dbToAmplitude,
  P as resolveValue,
  B as trimSilence
