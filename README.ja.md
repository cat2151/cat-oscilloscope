...
- オフセット%オーバーレイ（Offset %）- Issue #254調査中
  - 「今回の波形」パネルの右上に表示される、位相マーカーの4周期データ内での位置を示すグラフ
  - **用語説明**:
    - **4周期データ**: 表示ウィンドウ内の約4周期分の波形データ
    - **start offset**: 位相0マーカー（開始位置）が4周期データ内のどこにあるか（0-100%）
    - **end offset**: 位相2πマーカー（終了位置）が4周期データ内のどこにあるか（0-100%）
  - **表示内容**: 
    - 赤線（S）: start offset（位相0の相対位置、0-100%）
    - オレンジ線（E）: end offset（位相2πの相対位置、0-100%）
  - **スパイク検出**: 
    - フレーム間で1%を超える変化を検出すると、コンソールに診断情報を出力（仕様では1%以内）
    - 診断情報には以下が含まれます：
      - start/end offsetの値（4周期データ内での位置%）
      - 位相マーカーのサンプル位置と前フレームからの移動量
      - 表示ウィンドウの範囲と前フレームからの変化量
      - オフセット変化量（仕様では1%以内であるべき）
  - **目的**: 位相マーカーまたは表示ウィンドウのどちらが移動してスパイクを引き起こしているかを特定
