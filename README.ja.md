...
- オフセット%オーバーレイ（Δ Offset %）
  - 「今回の波形」パネルの右上に表示される、位相マーカーのフレーム間移動量を示すグラフ
  - **表示内容**: 
    - 赤線（S）: 位相0（開始マーカー）のフレーム間移動量
    - オレンジ線（E）: 位相2π（終了マーカー）のフレーム間移動量
    - ゼロライン（灰色）: 移動なしを示す基準線
  - **値の意味**: 
    - 0%近辺: 位相マーカーが安定して追跡されている（理想的な状態）
    - ±1%以内: 正常な追跡範囲（フレーム間の微小な変動）
    - ±5%を超える: 表示ウィンドウの変更や波形検出の大きな変化を示す
  - **注意**: 初回フレームやリセット直後は0%からスタートします
