...
- オフセット%オーバーレイ（Offset %）- Issue #254調査中
  - 「今回の波形」パネルの右上に表示される、位相マーカーの表示ウィンドウ内での位置を示すグラフ
  - **表示内容**: 
    - 赤線（S）: 位相0（開始マーカー）の表示ウィンドウ内での相対位置（0-100%）
    - オレンジ線（E）: 位相2π（終了マーカー）の表示ウィンドウ内での相対位置（0-100%）
  - **スパイク検出**: 
    - フレーム間で5%を超える変化を検出すると、コンソールに診断情報を出力
    - 診断情報には以下が含まれます：
      - 位相マーカーの絶対位置（サンプルインデックス）とその変化量
      - 表示ウィンドウの範囲とその変化量
      - 計算されたオフセット%とその変化量
