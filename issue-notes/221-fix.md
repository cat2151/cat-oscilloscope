...
- `PeakBacktrackWithHistory`: 厳密な1%制約でhistory付近のみを探索し、ゼロクロスが見つからない場合はhistoryを維持
  - `PeakBacktrackWithHistory`: Uses a strict 1% constraint around the history position and keeps the history if no zero-cross is found
- 上記以外のモード（`Hysteresis`, `BidirectionalNearest` など）は、現状すべて共通のフォールバックロジック（history 付近 1% 範囲でのゼロクロス探索 → 見つからなければセグメント全体検索）を使用する
  - All other modes (`Hysteresis`, `BidirectionalNearest`, etc.) currently share the same fallback logic: search for a zero-cross within 1% around the history position, and if not found, search the entire segment
