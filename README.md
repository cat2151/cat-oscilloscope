...
- **Library Support**: Available as an npm library for use in other projects (ESM/CJS compatible, full type definition support).
...
**Display Control**: Display/hide overlays (FFT spectrum, harmonic analysis, frequency transition plot) can be controlled with `setDebugOverlaysEnabled()`. You can also customize the layout with `setOverlaysLayout()`. For details, please refer to "Controlling Debug Overlay Display" and "Customizing Overlay Layout" in [LIBRARY_USAGE.md](./LIBRARY_USAGE.md).
...
**Example Usage**: To detect low frequencies between 20-50Hz, select STFT or CQT and set Buffer Size to 16x for optimal results.
...
- **Auto Gain**:
  ...
  - Can be toggled on/off with UI checkbox (default: on).
...
The microphone picks up all surrounding sounds, so environmental noise can affect the waveform as follows:

- **Mouse Click Sounds**: Mechanical sounds from mouse clicks will appear in the waveform. Especially when clicking the pause button with a mouse, the waveform may appear distorted.
...
- **Other Environmental Sounds**: Voices, indoor air conditioning sounds, and external noise can also appear in the waveform.

### Practical Tips

- **How to Pause**: Instead of mouse clicks, using the spacebar on a silent keyboard can minimize the impact on the waveform when pausing.
- **Sound Source Selection**: Since microphone input is susceptible to ambient noise, if you want to observe a noise-free waveform, it is recommended to use audio files such as WAV files.
